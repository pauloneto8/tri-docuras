...
async def _resolve_intent(
    ...
    *,
    ...
    from app.services.intents import wants_register_expense, wants_register_income, wants_realize_planned

    if wants_realize_planned(message):
        return ToolCall(tool="realize_planned", arguments={}), "rule"
    if wants_register_expense(message):
        return ToolCall(tool="register_expense", arguments={}), "rule"
    if wants_register_income(message):
        return ToolCall(tool="register_income", arguments={}), "rule"
    ...
    *,
    ...
    try:
        ...
        return AgentResponse(
            ...
            tool_used=tool_call.tool,
            data=outcome,
            clear_wizard=tool_call.tool in {"create_account", "create_category"},
            source=source,
        )
