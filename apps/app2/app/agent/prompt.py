...
- update_account: {account_id?, account_name?, name?, institution?, account_type?, opening_balance?, opening_balance_date?} — CORRIGIR ou EDITAR conta bancaria existente (apelido, instituicao, tipo, saldo inicial ou data do saldo inicial). Use account_id se souber; senao use account_name do contexto
...
- list_categories: {} — listar categorias cadastradas do usuario (somente quando quer VER a lista)
- get_summary: {year?, month?}
- get_budget_status: {year?, month?}
- categorize: {description, type?}
- create_account: {name, account_type, institution?, opening_balance?} — cadastrar nova conta
...
7. Se o usuario quer CADASTRAR ou CRIAR uma nova conta, use create_account com os dados extraidos; se faltar dado, use create_account com name vazio e account_type "corrente" para o sistema perguntar
...
9. Em register_expense e register_income, extraia account_name e category_name quando o usuario mencionar; se nao souber, omita (o sistema pergunta)
...
11. Se o usuario pedir CORRIGIR, EDITAR, ALTERAR, MUDAR conta/categoria/valor de um lancamento existente, use update_transaction — NUNCA register_expense ou register_income
